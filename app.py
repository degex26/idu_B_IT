...
def init_db():
    try:
        ...
        cur.close()
        conn.close()
        print("✅ Таблицы projects и users готовы")
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .subtitle { color: rgba(255,255,255,0.3); font-size: 14px; margin-bottom: 20px; }
        .form-group { margin-bottom: 16px; text-align: left; }
        .form-group label { color: rgba(255,255,255,0.5); font-size: 13px; display: block; margin-bottom: 4px; }
        .form-group input {
            width: 100%;
            padding: 10px 14px;
            border-radius: 12px;
        ...
        .form-group input:focus { outline: none; border-color: rgba(168,85,247,0.4); }
        .btn-submit {
            width: 100%;
            padding: 12px;
            border-radius: 12px;
        ...
        .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(124, 58, 237, 0.3); }
        .flash {
            padding: 10px 16px;
            border-radius: 10px;
            margin-bottom: 16px;
        ...
        .flash-success { background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.2); color: rgba(16, 185, 129, 0.8); }
        .flash-error { background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.2); color: rgba(239, 68, 68, 0.8); }
        .auth-link { color: rgba(255,255,255,0.3); font-size: 14px; margin-top: 16px; display: block; }
        .auth-link a { color: #a855f7; text-decoration: none; }
        .auth-link a:hover { text-decoration: underline; }
        .stars-left, .waves-right {
            position: fixed;
            top: 0;
            width: 60px;
            height: 100%;
        ...
        .waves-right { right: 10px; width: 80px; }
        .wave-line {
            position: absolute;
            width: 2px;
            height: 100%;
...
<body>
    ...
    <div class="card">
        ...
        <div class="auth-link">Нет аккаунта? <a href="/register">Зарегистрироваться</a></div>
    </div>
</body>
</html>
'''
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Регистрация — IDU B IT</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
        ...
        .form-group input:focus { outline: none; border-color: rgba(16,185,129,0.4); }
        .btn-submit {
            width: 100%;
            padding: 12px;
            border-radius: 12px;
        ...
        .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(16, 185, 129, 0.3); }
        .flash {
            padding: 10px 16px;
            border-radius: 10px;
            margin-bottom: 16px;
...
<body>
    ...
    <div class="card">
        ...
        <div class="auth-link">Уже есть аккаунт? <a href="/login">Войти</a></div>
    </div>
</body>
</html>
'''
...
# ============================================
# КОНТЕНТ ГЛАВНОЙ (только для авторизованных)
# ============================================
HTML_INDEX = '''
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .logo { font-size: 64px; margin-bottom: 10px; display: inline-block; animation: pulse 3s infinite ease-in-out; }
        @keyframes pulse {
        ...
        .subtitle { color: rgba(255,255,255,0.4); font-size: 18px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 16px; }
        .divider { width: 60px; height: 2px; background: linear-gradient(90deg, transparent, rgba(168,85,247,0.4), transparent); margin: 16px auto 20px; }
        .path-section {
            margin: 12px 0 16px;
        ...
        .path-point.start .point-icon { border-color: rgba(168,85,247,0.2); background: rgba(168,85,247,0.05); }
        .path-point.start .point-label { color: rgba(168,85,247,0.4); }
        .path-point.end .point-icon {
            border-color: rgba(0,200,255,0.2);
            background: rgba(0,200,255,0.05);
            animation: glowPulse 2s infinite ease-in-out;
        }
        .path-point.end .point-label { color: rgba(0,200,255,0.5); }
        @keyframes glowPulse {
        ...
        @keyframes rocketFly {
            ...
            50% { transform: translate(-50%, -75%) scale(1.1) rotate(4deg); }
        }
        .nav-links {
            display: flex;
            justify-content: center;
        ...
        .deploy-info .version { color: rgba(255,255,255,0.25); font-weight: 600; }
        .deploy-info .time { color: rgba(255,255,255,0.15); }
        .buttons {
            display: flex;
            flex-wrap: wrap;
            justify-content: center;
        ...
        .btn-primary:hover { transform: translateY(-3px); box-shadow: 0 8px 30px rgba(124, 58, 237, 0.4); }
        .btn-secondary {
            background: rgba(255, 255, 255, 0.04);
        ...
        .btn-danger:hover { background: rgba(239, 68, 68, 0.25); transform: translateY(-3px); }
        .status {
            margin-top: 24px;
        ...
        @media (max-width: 600px) {
            ...
            .path-point .point-icon { font-size: 20px; width: 38px; height: 38px; }
            .path-point .point-label { font-size: 10px; }
            .path-line { min-width: 20px; max-width: 60px; }
            .path-walker { font-size: 24px; }
            .path-section { padding: 12px 6px 8px; margin: 8px 0 12px; }
            .divider { margin: 12px auto 16px; }
    ...
    <div class="card">
        ...
        <div class="nav-links">
            ...
            {% if current_user.is_authenticated %}
                <a href="/logout" class="btn-danger" style="padding:4px 16px; border-radius:50px; font-size:12px;">🚪 Выйти</a>
            {% else %}
                <a href="/login">🔐 Войти</a>
            {% endif %}
        </div>
        {% if current_user.is_authenticated %}
            <div class="user-info">👋 Привет, {{ current_user.username }}!</div>
        {% endif %}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .subtitle { color: rgba(255,255,255,0.3); font-size: 14px; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 20px; }
        .divider { width: 60px; height: 2px; background: linear-gradient(90deg, transparent, rgba(168,85,247,0.4), transparent); margin: 12px auto 20px; }
        .nav-links {
            display: flex;
            justify-content: center;
            gap: 20px;
        ...
        .about-text strong { color: rgba(255,255,255,0.85); }
        .about-text .highlight { color: #a855f7; font-weight: 600; }
        .btn {
            padding: 12px 32px;
            border-radius: 50px;
            font-size: 15px;
        ...
        .deploy-info .version { color: rgba(255,255,255,0.2); font-weight: 600; }
        .deploy-info .time { color: rgba(255,255,255,0.1); }
        @media (max-width: 600px) {
            .card { padding: 32px 20px; border-radius: 24px; }
            h1 { font-size: 28px; }
            .logo { font-size: 36px; }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .subtitle { color: rgba(255,255,255,0.3); font-size: 14px; letter-spacing: 2px; text-transform: uppercase; margin-bottom: 20px; }
        .divider { width: 60px; height: 2px; background: linear-gradient(90deg, transparent, rgba(16,185,129,0.4), transparent); margin: 12px auto 20px; }
        .nav-links {
            display: flex;
            justify-content: center;
            gap: 20px;
        ...
        .project-form .btn-submit:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(16,185,129,0.3); }
        .project-list {
            display: flex;
            flex-direction: column;
            gap: 12px;
        ...
        .project-item:hover { background: rgba(255,255,255,0.04); border-color: rgba(16,185,129,0.2); }
        .project-item .name { font-size: 16px; font-weight: 600; color: rgba(255,255,255,0.85); }
        .project-item .description { font-size: 14px; color: rgba(255,255,255,0.5); margin-top: 4px; }
        .project-item .tech {
            font-size: 12px;
        ...
        .project-item .delete-btn:hover { color: #ef4444; }
        .empty-msg { color: rgba(255,255,255,0.2); font-size: 14px; text-align: center; padding: 20px 0; }
        .btn {
            padding: 12px 32px;
            border-radius: 50px;
            font-size: 15px;
        ...
        @media (max-width: 600px) {
            ...
            .stars-left, .waves-right { display: none; }
            .deploy-info { font-size: 9px; }
            .project-item { padding: 12px 16px; }
        }
    </style>
</head>
<body>
    ...
    <div class="card">
        ...
        {% if not current_user.is_authenticated %}
            <div class="flash flash-error">🔒 Для доступа к проектам необходимо <a href="/login" style="color:#a855f7;">войти</a></div>
        {% else %}
            {% if message %}
